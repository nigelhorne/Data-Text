...
diag("Data::Text->set test case created by https://github.com/nigelhorne/App-Test-Generator");
...
foreach my $case (@{fuzz_inputs()}) {
	...
	if(my $status = (delete $case->{'_STATUS'} || delete $output{'_STATUS'})) {
		if($status eq 'DIES') {
			dies_ok { $result = $obj->set($input); } sprintf($mess, 'dies');
		} elsif($status eq 'WARNS') {
			warnings_exist { $result = $obj->set($input); } qr/./, sprintf($mess, 'warns');
		} else {
			lives_ok { $result = $obj->set($input); } sprintf($mess, 'survives');
		}
	} else {
		lives_ok { $result = $obj->set($input); } sprintf($mess, 'survives');
	}
...
# --- Static Corpus Tests ---
warnings_exist { Data::Text::set(undef, undef, undef) } qr/./, 'set(, , ) warns';
